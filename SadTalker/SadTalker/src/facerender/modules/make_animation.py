...
def stream_animation(source_image, audio_path, m3u8_path, source_semantics, target_semantics,
                     generator, kp_detector, he_estimator, mapping, 
                     yaw_c_seq=None, pitch_c_seq=None, roll_c_seq=None,
                     use_exp=True, use_half=False, tsSegmentLength = 5):
    second_half = []
    ...
    with torch.no_grad():
        ...
        frameBatches = target_semantics.shape[1]
        with m3u8Streamer(audio_path, m3u8_path, tsSegmentLength) as streamer:
            for frame_idx in tqdm(range(frameBatches), 'Face Renderer (1st half):'):
                target_semantics_frame = target_semantics[:, frame_idx]
                he_driving = mapping(target_semantics_frame)
                if yaw_c_seq is not None:
                    he_driving['yaw_in'] = yaw_c_seq[:, frame_idx]
                ...
                image = out['prediction'].squeeze(1)[0]
                stream_frame_to_batch(streamer, image)
                second_half.append(out['prediction'].squeeze(1)[1])
    
            for i in tqdm(range(len(second_half)), "Face Renderer (2nd half)"):
                image = second_half[i]
                stream_frame_to_batch(streamer, image)

def stream_frame_to_batch(streamer, image):
    image = image.permute(1, 2, 0).cpu().numpy()
    image = (image - image.min()) / (image.max() - image.min()) * 255 
    image = image.astype(np.uint8)
    pil_image = Image.fromarray(image)
    streamer.streamToFFMPEG(pil_image)
