...
def generate_blink_seq_randomly(num_frames):
    ...
    frame_id = 0
    while frame_id in range(num_frames):
        start_range = min(10, num_frames)
        end_range = min(int(num_frames / 2), 70)

        if start_range < end_range:
            start = random.choice(range(start_range, end_range))
        else:
            # Handle the case where the range would be invalid
            start = start_range  # or any other logic that suits your needs
