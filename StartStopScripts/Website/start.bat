...
rem ####################################################################
rem ##### RUN EXISITING CONTAINER IF IT EXISTS #########################Add commentMore actions
rem ####################################################################
...
set cmd=%cmd% -e OpenClone_SadTalker_HostAddress=%OpenClone_SadTalker_HostAddress%
...
rem Write command to temporary batch file to avoid parsing issues
echo %cmd% > "%TEMP%\openclone-website-docker.bat"
wt --window 0 new-tab -p "OpenCloneWebsite" cmd /k "call \"%TEMP%\openclone-website-docker.bat\" && pause"